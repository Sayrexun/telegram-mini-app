  ...
  modal.innerHTML = `
    <div class="sheet">
      ...
      <div class="cart-actions">
        ...
        <button class="primary" onclick="openRequestForm()">Оставить заявку</button>
      </div>
    </div>`;
...
window.openRequestForm = function(){
  ...
  const items = Object.entries(counts).map(([id,qty])=>{ const p=data.products.find(x=>x.id==id); return p?{id:p.id,name:p.name,qty,price:Number(p.price)}:null; }).filter(Boolean);
  if(!items.length){toast("Корзина пуста");return;}
  const total=items.reduce((s,x)=>s+x.price*x.qty,0);
  const modal=document.querySelector('.modal');
  if(modal) modal.remove();
  const m=document.createElement('div');m.className='modal';m.innerHTML=`<div class="sheet"><h2>Новая заявка</h2><p>Товары: ${items.map(x=>esc(x.name)+" × "+x.qty).join(', ')}</p><div class="product-bottom"><b>Итого</b><b class="price">${total.toFixed(0)} zł</b></div><textarea id="requestComment" placeholder="Комментарий (необязательно)" style="width:100%;min-height:90px;margin:14px 0;padding:12px;border-radius:10px;border:1px solid #30293b;background:#0d0b12;color:#fff"></textarea><div class="cart-actions"><button class="secondary" onclick="this.closest('.modal').remove()">Отмена</button><button class="primary" onclick="submitRequest()">Отправить заявку</button></div></div>`;document.body.appendChild(m);
};
window.submitRequest = async function(){
  const user=window.Telegram?.WebApp?.initDataUnsafe?.user;
  if(!user?.id){toast("Откройте каталог внутри Telegram");return;}
  const counts={};cart.forEach(id=>counts[id]=(counts[id]||0)+1);
  const items=Object.entries(counts).map(([id,qty])=>{const p=data.products.find(x=>x.id==id);return p?{id:p.id,name:p.name,qty,price:Number(p.price)}:null}).filter(Boolean);
  const total=items.reduce((s,x)=>s+x.price*x.qty,0);
  try{const r=await fetch('/api/requests',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({user,items,total,comment:document.getElementById('requestComment')?.value||''})});if(!r.ok)throw new Error();const d=await r.json();cart=[];document.getElementById('cartCount').textContent='0';document.querySelector('.modal')?.remove();toast('Заявка №'+d.id+' отправлена');}catch{toast('Не удалось отправить заявку')}
};
