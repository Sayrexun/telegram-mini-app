...
app.post("/api/admin/products", auth, (req, res) => {
  ...
  db.prepare("INSERT OR IGNORE INTO categories(name) VALUES(?)").run(category.trim());
  const info = db.prepare("INSERT INTO products(name,category,price,image,description) VALUES(?,?,?,?,?)").run(name.trim(), category.trim(), Number(price), image, description);
...
app.put("/api/admin/products/:id", auth, (req, res) => {
  ...
  if (!String(name || "").trim() || !String(category || "").trim() || !Number.isFinite(Number(price))) return res.status(400).json({ error: "Invalid data" });
  db.prepare("INSERT OR IGNORE INTO categories(name) VALUES(?)").run(category.trim());
  db.prepare(`UPDATE products SET name=?,category=?,price=?,image=?,description=?,active=? WHERE id=?`).run(name.trim(), category.trim(), Number(price), image, description, active ? 1 : 0, req.params.id);
  res.json({ ok: true });
});
...
app.post("/api/admin/categories", auth, (req, res) => {
  ...
  res.json({ ok: true, categories: db.prepare("SELECT * FROM categories ORDER BY id").all() });
});
...
app.get("/api/admin/users", auth, (req, res) => {
  res.json(db.prepare(`SELECT u.*, COUNT(r.id) request_count, COALESCE(SUM(r.total),0) total_amount
...
app.get("/api/admin/requests", auth, (req, res) => {
  res.json(db.prepare(`SELECT r.*, u.telegram_id, u.username, u.first_name, u.last_name, u.phone
